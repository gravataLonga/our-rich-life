<x-card>
    <div class="flex items-start justify-between mb-4">
        ...
        <div class="bg-green-100 text-green-700 px-4 py-2 rounded-lg font-bold text-lg">
            {{ round($recording->total_amount * 100 / $recording->recordable->goal->value(), 2)  }} %
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
            ...
            <p class="text-2xl font-bold text-gray-800">{{ new \OurRichLife\Money($recording->total_amount)->format('€') }}</p>
        </div>
        <div>
    ...
    <div class="bg-gray-200 rounded-full h-2 overflow-hidden">
        <div class="progress-fill bg-linear-to-r from-green-400 to-green-600 h-full rounded-full" style="width: {{  clamp(round($recording->total_amount * 100 / $recording->recordable->goal->value(), 2), 0, 100) }}%"></div>
    </div>
